...
/* Constructors and destructor */
    ...
    void BST::copyTree(BSTNode* & newNode, BSTNode* current) const{
    if(current != nullptr){
         newNode = new BSTNode(current->element);
         copyTree(newNode->left, current->left);
         copyTree(newNode->right, current->right);
    }
      }
      ...
      BSTNode* newNode = new BSTNode(newElement);
         if (root == nullptr) {
             ...
             if (!insertR(newNode, root)) {
                 ...
                 throw ElementAlreadyExistsException("insert()");
             }
         }
     }
